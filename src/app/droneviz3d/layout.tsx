...
export default function DroneViz3DLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white font-sans selection:bg-cyan-500/30">
      ...
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#09090b]/70 backdrop-blur-2xl border-b border-white/[0.06]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14">
            <Link href="/droneviz3d" className="flex items-center gap-2.5 group">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-[11px] font-black tracking-tight shadow-lg shadow-cyan-500/20">
                DV
              </div>
            ...
            <div className="hidden md:flex items-center gap-3">
              {isProcessing && (
                <span className="flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/10 text-cyan-400 text-[11px] font-medium border border-cyan-500/20">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
                  Processing
                </span>
              )}
              {processingComplete && !isProcessing && (
                <span className="flex items-center gap-2 px-3 py-1 rounded-full bg-green-500/10 text-green-400 text-[11px] font-medium border border-green-500/20">
                  <span className="w-1.5 h-1.5 rounded-full bg-green-400" />
                  Ready
                </span>
              )}
              <span className="px-2.5 py-1 rounded-md bg-white/[0.04] text-white/30 text-[11px] font-mono">
                SIH 2026
              </span>
            </div>
      ...
      <footer className="relative z-10 border-t border-white/[0.04] mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-[12px] text-white/20">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 rounded bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-[8px] font-black">DV</div>
            <span>DroneViz3D &mdash; Team ByteCraft &mdash; SIH26158</span>
          </div>
          <span>Single-Pass Drone Video to Accurate 3D Model Generation</span>
        </div>
      </footer>
    </div>
  )
